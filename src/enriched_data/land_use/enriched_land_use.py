...
def create_landuse_network(gdf):
    """
    Cria um grafo de rede para análise de conectividade entre polígonos de uso do solo.
    
    Args:
        gdf (geopandas.GeoDataFrame): Dados de uso do solo classificados
        
    Returns:
        tuple: (gdf atualizado, grafo de rede)
    """
    logger.info("Criando grafo de rede para análise de conectividade de uso do solo")
    ...
    try:
        ...
        # Adicionar nós para cada polígono
        for idx, row in result.iterrows():
            G.add_node(idx, 
                       categoria=row['land_category'], 
                       area=row['area_km2'],
                       pos=(row.geometry.centroid.x, row.geometry.centroid.y))
        
        # Adicionar arestas para polígonos adjacentes
        for i, (idx1, row1) in enumerate(tqdm(result.iterrows(), total=len(result), desc="Construindo grafo")):
            for idx2, row2 in result.iloc[i+1:].iterrows():
                # Verificar se os polígonos são adjacentes (compartilham bordas)
                if row1.geometry.touches(row2.geometry):
                    # Adicionar aresta com dados
                    G.add_edge(idx1, idx2, 
                               same_category=(row1['land_category'] == row2['land_category']),
                               categories=f"{row1['land_category']}-{row2['land_category']}")
        
        # Calcular métricas de centralidade
        logger.info("Calculando métricas de centralidade da rede")
        
        # Betweenness centrality (identifica polígonos importantes para conectividade)
        betweenness = nx.betweenness_centrality(G, k=min(500, len(G)))
        
        # Degree centrality (número normalizado de conexões)
        degree = nx.degree_centrality(G)
        
        # Adicionar métricas de centralidade ao GeoDataFrame
        result['betweenness'] = result.index.map(betweenness)
        result['degree_centrality'] = result.index.map(degree)
        
        # Identificar polígonos críticos para conectividade (alto betweenness)
        threshold = np.percentile([b for b in betweenness.values() if b > 0], 90)
        result['is_critical_connector'] = result['betweenness'] > threshold
        
        # Identificar clusters/componentes conectados
        components = list(nx.connected_components(G))
        
        # Mapear cada nó para seu componente
        node_to_component = {}
        for i, comp in enumerate(components):
            for node in comp:
                node_to_component[node] = i
        
        # Adicionar ID do componente ao GeoDataFrame
        result['component_id'] = result.index.map(node_to_component)
        
        # Armazenar informações de rede como atributos
        result.attrs['network_stats'] = {
            'n_nodes': len(G),
            'n_edges': G.number_of_edges(),
            'n_components': len(components),
            'largest_component_size': len(max(components, key=len)),
            'network_density': nx.density(G),
            'critical_connectors': int(result['is_critical_connector'].sum())
        }
        
        logger.info(f"Grafo criado com {len(G)} nós e {G.number_of_edges()} arestas")
        
        return result, G
        
    except Exception as e:
        logger.error(f"Erro ao criar grafo de rede: {str(e)}")
        return result, None
...
def main():
    ...
    # 6. Calcular matriz de adjacência
    logger.info("Calculando matriz de adjacência")
    enriched_gdf, adjacency_matrix = create_landuse_adjacency(enriched_gdf)
    ...
    # 8. Gerar visualizações melhoradas
    logger.info("Gerando visualizações melhoradas")
