...
# Verificar se o script está sendo executado no Colab ou localmente
import sys
import os

# Variável global para indicar o ambiente
IS_COLAB = 'google.colab' in sys.modules
IS_NOTEBOOK = False

# Configuração para import de tqdm e display
try:
    ...
    IS_NOTEBOOK = False
    # Função alternativa para display quando não estamos em um notebook
    def display(obj):
...
# Define a função check_jupyter para verificar se estamos em ambiente Jupyter
def check_jupyter():
    try:
...
drive.mount('/content/drive')
        ...
        print("Não foi possível montar o Google Drive. Verifique se o módulo google.colab está disponível.")
        BASE_DIR = "."
else:
    # Se estiver executando localmente, usar caminhos locais
    # Você pode adaptar estes caminhos conforme necessário
    print("Executando em ambiente local...")
    
    # Tentar obter o caminho do diretório de trabalho atual
    try:
        # Usar o caminho absoluto do arquivo atual como base
        SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
        # Subir um nível para chegar no diretório raiz do projeto
...
# Definir os diretórios de dados, saída e relatórios
...
data_dir = DATA_DIR  # Usando a variável definida anteriormente

# Lista de arquivos GPKG disponíveis
...
def explore_gpkg(gpkg_path):
    """
    Explora um arquivo GeoPackage para obter informações sobre suas camadas.
    
    Args:
        gpkg_path: Caminho para o arquivo GeoPackage
        
    Returns:
        DataFrame com informações sobre as camadas
    """
...
output.enable_custom_widget_manager()
        ...
        print("Widgets do Colab não disponíveis.")
...
# Comentário sobre a estrutura de notebooks de integração de edifícios e uso do solo
"""
Descrição dos notebooks de integração:
- Sobreposicao_Espacial.ipynb: Realiza análise de sobreposição entre edifícios e uso do solo
- Categorizacao_Funcional_Edificios.ipynb: Categoriza os edifícios por função
- Analise_Conformidade_Uso.ipynb: Analisa a conformidade dos edifícios com o uso do solo
"""
...
if isinstance(buildings_with_landuse, dict):
    print(f"Dados carregados como dicionário com {len(buildings_with_landuse)} chaves")
    print(f"Chaves disponíveis: {list(buildings_with_landuse.keys())}")
    
    # Se for um dicionário, extrair o GeoDataFrame de edifícios
    found_gdf = False
    
    # Primeiro, procurar por uma chave específica de edifício
    building_keys = ['buildings', 'edificios', 'edificações', 'building']
    ...
    if not found_gdf:
        for key, value in buildings_with_landuse.items():
            if isinstance(value, gpd.GeoDataFrame):
                # Verificar se parece um GeoDataFrame de edifícios
...
create_demo_data = False  # Altere para True se quiser usar dados de demonstração
if create_demo_data and (buildings_gdf.empty or 'geometry' not in buildings_gdf.columns):
    print("\nCriando dados de demonstração para testes...")
    from shapely.geometry import Point
    ...
    print("Usando dados de demonstração para permitir o funcionamento do pipeline.")
    print(f"Dados de demonstração criados com {len(buildings_gdf)} registros.")
...
def explore_building_attributes(gdf):
    ...
    if gdf.empty:
        print("GeoDataFrame está vazio. Não há dados para explorar.")
        return
...
if isinstance(buildings_gdf, gpd.GeoDataFrame) and not buildings_gdf.empty:
    ...
    print("Não há dados de edifícios para explorar")
...
# Exibir amostra do resultado
    ...
    # Analisar os resultados da categorização - APENAS se tivermos dados válidos
    if isinstance(categorized_buildings, gpd.GeoDataFrame) and not categorized_buildings.empty and 'categoria_funcional' in categorized_buildings.columns:
        try:
            ...
            if 'analyze_categorization_results' in globals():
                ...
                print("Função analyze_categorization_results não está definida. Pulando análise visual de categorização.")
            ...
            try:
                if 'map_categorized_buildings' in globals():
                    ...
                    print("Função map_categorized_buildings não está definida. Pulando visualização de mapa.")
            except Exception as e:
                print(f"Erro ao mapear edifícios categorizados: {e}")
        except Exception as e:
            print(f"Erro ao analisar resultados da categorização: {e}")
    else:
        print("Dados categorizados não possuem colunas necessárias para análise visual")
else:
    print("Não há dados de edifícios válidos para categorização")
...
# Função para criar um mapa dos edifícios categorizados
def map_categorized_buildings(gdf, figsize=(15, 15), sample_size=1000):
    ...
    """
    print("INFO: Função map_categorized_buildings executando versão simplificada.")
...
def finalize_categorization(gdf, existing_class_col=None):
    ...
    if unclassified_count > 0:
        ...
        print(f"Edifícios ainda sem classificação: {unclassified_count} ({unclassified_percent:.2f}%)")
    else:
        print(f"Todos os edifícios foram classificados com sucesso!")
    ...
    if unclassified.any():
        ...
        # Evitar divisão por zero novamente
        if unclassified_count > 0:
            unclassified_percent = unclassified_count / total_count * 100
...
def validate_categorization(gdf, n_samples=5):
    ...
    Returns:
        dict: Métricas de categorização
    """
    if gdf is None:
        print("GeoDataFrame é None. Não é possível validar.")
        return {'total': 0, 'categorized': 0, 'percent_categorized': 0}
        
    if len(gdf) == 0:
        print("GeoDataFrame vazio. Não é possível validar.")
        return {'total': 0, 'categorized': 0, 'percent_categorized': 0}
    
    # Criar uma cópia para não modificar o original
    gdf_copy = gdf.copy()
    
    # Verificar e adicionar as colunas necessárias individualmente
    ...
    if missing_columns:
        print(f"AVISO: As seguintes colunas não existem no DataFrame e foram adicionadas: {missing_columns}")
    
    # Calcular métricas de categorização
    total = len(gdf_copy)
    categorized = 0
    ...
    print(f"Total de edifícios: {total}")
    print(f"Edifícios categorizados: {categorized} ({percent_categorized:.2f}%)")
    
    # Selecionar colunas para exibir na validação
    display_columns = ['geometry']
    
    # Adicionar as colunas de categorização se existirem
    if 'building_class_enhanced' in gdf_copy.columns:
    ...
    # Adicionar outros atributos que podem ajudar na validação
    for col in ['name', 'building', 'amenity', 'shop', 'aeroway', 'tourism']:
        if col in gdf_copy.columns:
            display_columns.append(col)
    
    # Mostrar amostras de cada categoria para validação
    if 'building_class_enhanced' in gdf_copy.columns:
        # Filtrar para obter apenas valores não nulos
        ...
        if non_null_mask.any():
            ...
            if len(categories) > 0:
                print(f"\nExibindo até {n_samples} amostras de cada categoria ({len(categories)} categorias no total)")
                
                for cat in categories:
                    subset = gdf_copy[gdf_copy['building_class_enhanced'] == cat]
                    print(f"\nCategoria: {cat} ({len(subset)} edifícios)")
                    
                    if len(subset) > 0:
                        # Tentar obter amostras aleatórias com tratamento de erro
                        try:
                            # Garantir que não tentamos obter mais amostras do que existem
                            ...
                            if sample_n > 0:
                                ...
                                print("Não há edifícios suficientes para amostragem.")
                        except Exception as e:
                            print(f"Erro ao exibir amostra: {e}")
            else:
                print("Nenhuma categoria encontrada nos dados.")
        else:
            print("Nenhum edifício foi categorizado (todas as entradas são nulas).")
    else:
        print("Coluna 'building_class_enhanced' não disponível para mostrar categorias.")
...
print("\nNão foi possível gerar estatísticas finais - dados insuficientes ou inválidos")
...
if isinstance(final_categorized_buildings, gpd.GeoDataFrame) and not final_categorized_buildings.empty:
    try:
        ...
        print("Função explore_landuse_data não está definida. Pulando análise de zoneamento.")
...
conformity_rules = define_conformity_rules()
        ...
        # Verificar se a análise de conformidade foi bem-sucedida
...
# Identificar anomalias e padrões espaciais
                ...
                print(f"\nA análise de conformidade foi concluída com sucesso!")
                if gpkg_path:
        ...
        print(f"Erro ao realizar análise de conformidade: {e}")
else:
    print("Não foi possível realizar a análise de conformidade devido à falta de dados de zoneamento")

# Se não encontrou dados válidos, criar um GeoDataFrame de exemplo para testes
...
# Adicionar implementações mínimas para funções ausentes
# Estas implementações apenas evitam erros e não executam a funcionalidade real

def explore_landuse_data(gdf):
    """Versão temporária para evitar erros."""
    print("INFO: Função explore_landuse_data não implementada completamente.")
...
def define_conformity_rules():
    """Versão temporária para evitar erros."""
    print("INFO: Função define_conformity_rules não implementada completamente.")
    return {"regras_vazias": True}  # Retorna dicionário vazio de regras

def analyze_conformity(gdf, landuse_col, rules):
    """Versão temporária para evitar erros."""
    print("INFO: Função analyze_conformity não implementada completamente.")
    return gdf.copy(), {}, {}  # Retorna GeoDataFrame sem modificações e dicionários vazios

def map_conformity(gdf):
    """Versão temporária para evitar erros."""
    print("INFO: Função map_conformity não implementada completamente.")
    # Não faz nada, apenas evita erros

def analyze_conformity_patterns(gdf, landuse_col):
    """Versão temporária para evitar erros."""
    print("INFO: Função analyze_conformity_patterns não implementada completamente.")
    # Não faz nada, apenas evita erros

def calculate_conformity_indices(gdf):
    """Versão temporária para evitar erros."""
    print("INFO: Função calculate_conformity_indices não implementada completamente.")
    return {}  # Retorna dicionário vazio

def identify_anomalies(gdf, landuse_col):
    """Versão temporária para evitar erros."""
    print("INFO: Função identify_anomalies não implementada completamente.")
    # Não faz nada, apenas evita erros

def save_conformity_analysis(gdf):
    """Versão temporária para evitar erros."""
    print("INFO: Função save_conformity_analysis não implementada completamente.")
    gpkg_path = None
    pickle_path = None
    indices_path = None
...
def analyze_categorization_results(gdf, stats):
    """Versão temporária para evitar erros."""
    print("INFO: Função analyze_categorization_results não implementada completamente.")
    # Versão básica - apenas mostra contagem de categorias se disponível
    if 'categoria_funcional' in gdf.columns:
        categories = gdf['categoria_funcional'].value_counts()
        print("\nDistribuição de categorias:")
        print(categories)
    # Não faz nada mais, apenas evita erros

# Importações necessárias para evitar erros
try:
    import seaborn as sns
except ImportError:
    print("Biblioteca seaborn não está disponível. Algumas visualizações serão limitadas.")
...
# Verificar se tqdm está disponível para barras de progresso
try:
    from tqdm.notebook import tqdm
except ImportError:
    try:
        ...
        # Criar uma versão simples de tqdm se não estiver disponível
